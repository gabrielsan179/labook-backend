...
//         }
//         const [userId] : UserDB[] | undefined[] = await db("users")
//             .where("id", "=", `${id}`)
//         if (userId) {
//             res.status(400)
...
//         }
//         const [userEmail] : UserDB[] | undefined[] = await db("users")
//             .where("email", "=", `${email}`)
//         if (userEmail) {
//             res.status(400)
...
//             throw new Error("Email precisa ser uma string")
//         }
//         const [userExist] : UserDB[] | undefined[] = await db("users")
//             .where("email", "=", `${email}`)
//         if (!userExist) {
//             res.status(400)
...
//     try {
//         const allPosts : PostDB[] = await db("posts")
//         const result: PostModel[] = []
//         for(let post of allPosts) {
//             const [ posts ]: PostModel[] = await db("posts")
//             .select(
//                 "posts.id",
//                 "posts.content",
//                 "posts.likes",
...
//         }
//         const [postExist]: PostDB[] | undefined[] = await db("posts")
//         .where("id", "=", `${id}`)
//         if (!postExist) {
//             res.status(400)
